...
// Force the custom (Glassmorphism) title bar. ElectronMainApplication.getTitleBarStyle()
// prioritizes the "frame" value persisted by electron-store in the user's config.json over
// the "window.titleBarStyle" preference below, so a stale native-frame session (e.g. one
// saved before this preference existed) permanently overrides "window.titleBarStyle: custom".
// This env var short-circuits that check unconditionally.
